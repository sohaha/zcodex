...
- 受影响数据/模式：原生 memory 的线程/阶段状态 gating；`zmemory` 默认数据库路径与 workspace key 语义；配置 schema 与文档中的 memory/zmemory 说明。
- 受影响用户界面/行为：默认同时开启两套 memory；关闭某一套不会再影响另一套；模型 prompt 中对 memory 与 `zmemory` 的指引会分离；`zmemory` 默认不再按 workspace 隔离路径落库。
...
- 目标：把原生 memory prompt 与 `zmemory` prompt 独立出来，并将 `zmemory` 默认路径策略改为全局根。
- 交付物：独立 prompt 模板/注入逻辑、更新后的 `zmemory` 路径解析实现、相关 core/zmemory 测试与快照更新。
- 完成条件：prompt 文案不再混淆只读/可写语义；`zmemory` 在无显式配置时默认解析到全局根路径；相关系统视图、说明与测试和新策略一致。
...
- 手动检查：检查 `codex-rs/zmemory/README.md`、`docs/config.md` 与相关 prompt/feature 文案是否清楚区分原生 memory 与 `zmemory`；检查默认 `zmemory` 路径说明是否改为全局根模式。
- 未执行的验证（如有）：无。
