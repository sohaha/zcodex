---
name: upgrade-rtk
description: Upgrade the Codex-curated RTK integration used by this repo. Use when syncing embedded `rtk` subcommands to a newer upstream `rtk-ai/rtk` release or PR, updating `.version/rtk.toml`, compact prompt guidance, alias behavior, and `codex-rs/cli` regression coverage.
---

# Upgrade RTK

Use this skill to upgrade the repository's embedded RTK baseline in a controlled way without re-importing upstream bootstrap, hook, analytics, or installer features.
...
2. Inspect the current embedded implementation before touching code:
   - `/workspace/codex-rs/rtk/src/lib.rs`
   ...
   - `/workspace/codex-rs/core/templates/compact/rtk_instructions.md`
...
- Keep the embedded `rtk` crate focused on operational wrappers; remove obsolete compatibility paths instead of leaving dead meta-command logic around.
...
- Do not make `core` prompt templates depend on `codex-rs/rtk` hook/init assets; keep Codex prompt guidance in `core/templates/compact/`.
