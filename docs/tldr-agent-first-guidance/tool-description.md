...
- 当问题同时包含“结构理解 + 事实核对”时，默认先用 `tldr` 建立结构视图，再读取少量源码验证默认值、feature gate、文档或测试结论。
...
- 如果问题是混合类，先做一次轻量 `tldr`（如 `context` / `impact` / `semantic`），再去读精确源码
- 如果 `tldr` 返回 `source=local` 或 `degradedMode.is_degraded=true`，应把结果视为“降级成功”
- 如果 `structuredFailure` 存在，先根据 `error_type` 判断是否值得重试

## 起手式建议

- 结构化代码问题：先 `tldr`，再决定是否补充文件阅读。
- 混合问题：先 `tldr` 建结构，再读源码验事实。
- 纯事实核对问题：直接读配置、源码、测试或文档。
- 如果已经开始为结构化问题连续 grep/read 多个文件，应该回退并尝试 `tldr`，除非用户明确要求 raw grep/read。
