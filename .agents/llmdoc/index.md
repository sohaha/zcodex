...
- `.agents/llmdoc/memory/reflections/2026-04-28-auth-401-should-fallback-without-retrying-the-same-provider.md`：provider 返回 `401` 时，应允许 request fallback 快速切到下一个 provider，但不能继续在同一个 provider 上做 sampling retry；`401` fallback 与 `401` retry 必须拆成两套判定，并用经过 `run_sampling_request()` 的集成测试锁住。
...
- `.agents/llmdoc/memory/reflections/2026-04-28-responses-custom-tool-provider-compat-must-retry-at-stream-consumer-boundary.md`：Responses 兼容 provider 对 `apply_patch` 的 `type=custom` 拒绝可能在流内部才变成 `CodexErr::InvalidRequest`；同 provider 自愈必须同时覆盖 request 建立点和 `ResponseStream` 消费边界，且能力学习要来自 provider 实际报错，不要根据官方域名/URL 猜测。
