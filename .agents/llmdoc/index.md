...
- `.agents/llmdoc/memory/reflections/2026-04-24-ztldr-search-timeout-must-separate-full-text-budget-from-structural-heavy-actions.md`：`ztldr search` 的全文扫描延迟画像与结构/语义分析不同，daemon IO timeout 不应继续共用同一个 heavy bucket；先在 command-specific timeout 分配层修复，再决定是否深入搜索实现优化。
