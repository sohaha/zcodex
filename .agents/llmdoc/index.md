...
- `.agents/llmdoc/memory/reflections/2026-04-22-mcp-ztldr-input-schema-must-wrap-top-level-oneof-in-object.md`：Responses API 已经修成顶层 object 并不代表 `ztldr` schema 全链路安全；`mcp-server` 若仍手写裸顶层 `oneOf`，一旦被桥接成 function tool 仍会被上游按“不是 `type: object`”拒绝，测试也要锁住顶层 `type` 与公共 `required`。
