...
## 这次学到的
- `ztldr` 这类 action-by-action 契约工具，不能只在 MCP schema 修 `oneOf`；还必须检查 Responses API 的共享工具序列化入口是否会把顶层 `oneOf` 压平。否则本地 schema 看起来正确，真正发给模型的 contract 仍然是错的。
...
- `codex-rs/tools/src/tool_spec.rs`
  - `ztldr` 参数 schema 改为 action 级 `oneOf`。
  - Responses API 序列化时为 `ztldr` 保留顶层 `oneOf`，不再被共享归一化逻辑压平。
  - 工具描述不再推荐同样缺 `language` 也跑不通的 action。
- `codex-rs/mcp-server/src/tldr_tool.rs`
  - MCP `inputSchema` 与工具描述同步到同一份 contract。
...
4. 最后用请求级测试核对真实外发 `tools[].parameters` 仍然保留 `oneOf`。

## 验证备注
- `cargo test -p codex-tools create_tldr_tool_exposes_decision_guidance_and_current_action_surface -- --exact`
- `cargo test -p codex-tools create_tools_json_for_responses_api_preserves_tldr_one_of_constraints -- --exact`
