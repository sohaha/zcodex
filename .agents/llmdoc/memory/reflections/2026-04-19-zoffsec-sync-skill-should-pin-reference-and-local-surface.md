# 2026-04-19 zoffsec 上游同步技能要同时钉住上游参考面和本地保留面

## 背景

- 本仓的 `codex zoffsec` 子命令最初就是参考 `ryfineZ/codex-session-patcher` 落地的。
- 但本地实现只吸收了其中的 Codex 相关工作流：
  - `codex zoffsec` 启动时注入 offsec 指令
  - rollout refusal 清理
  - zoffsec 会话的 clean-then-resume
...
- 本地保留面也要显式列出，否则后续同步时容易把“当前架构选择”误判成“落后于 upstream 的缺口”。本次确认需要默认保留的有：
  - 原生 `codex zoffsec` 子命令，而不是 Python 安装器
  - `zoffsec_config.rs` 的 base-instructions 注入
  - `rollout/src/patch.rs` 的显式 clean
  - `tui/src/zoffsec_resume.rs` 的显式 clean-then-resume
- `STATE.md` 和 checklist 应与 skill 同时创建，不要等第一次真实同步时再临时补；否则第一次执行时容易把“怎么同步”与“这次同步到了哪”混在一起。

## 落地做法

- 新增了 repo 专用技能 `.codex/skills/sync-zoffsec-reference/`。
