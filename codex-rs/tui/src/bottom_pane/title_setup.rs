...
impl TerminalTitleItem {
    pub(crate) fn description(self) -> &'static str {
        match self {
            ...
            TerminalTitleItem::FiveHourLimit => "5 小时用量限额剩余（不可用时省略）",
            TerminalTitleItem::WeeklyLimit => "每周用量限额剩余（不可用时省略）",
            TerminalTitleItem::CodexVersion => "Codex 应用版本",
            TerminalTitleItem::UsedTokens => "会话已用 token 数（为零时省略）",
            TerminalTitleItem::TotalInputTokens => "会话总输入 token 数",
            TerminalTitleItem::TotalOutputTokens => "会话总输出 token 数",
            TerminalTitleItem::SessionId => "当前会话标识（会话启动前省略）",
            TerminalTitleItem::FastMode => "是否处于快速模式",
