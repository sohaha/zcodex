...
impl ChatComposer {
    ...
    pub(super) fn history_search_footer_line(&self) -> Option<Line<'static>> {
        ...
        let mut line = Line::from(vec!["历史搜索：".dim(), search.query.clone().cyan()]);
        match search.status {
            ...
            HistorySearchStatus::NoMatch => line.push_span("  未找到匹配".red()),
        }
        Some(line)
    }
    ...
    pub(super) fn history_search_cursor_pos(&self, area: Rect) -> Option<(u16, u16)> {
        ...
        let prompt_width = Line::from("历史搜索：").width() as u16;
