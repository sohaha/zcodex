...
async fn plugins_popup_loading_state_snapshot() {
    ...
    let popup = normalize_ui_text(&render_bottom_popup(&chat, 100));
    assert!(
...
async fn plugins_popup_search_no_matches_and_backspace_restores_results() {
    ...
    let no_matches = normalize_ui_text(&render_bottom_popup(&chat, 100));
    assert!(
        no_matches.contains("zzz"),
        "expected popup to show the typed search query, got:\n{no_matches}"
    );
    assert!(
        no_matches.contains("无匹配结果"),
...
async fn apps_popup_stays_loading_until_final_snapshot_updates() {
    ...
    let normalized_before = normalize_ui_text(&before);
    assert!(
        normalized_before.contains("应用正在加载已安装和可用的应用"),
    ...
    let normalized_after = normalize_ui_text(&after);
    assert!(
        normalized_after.contains("共2个可用应用"),
        "expected refreshed apps popup snapshot, got:\n{after}"
    );
    assert!(
        normalized_after.contains("已安装2个"),
        "expected refreshed apps popup snapshot, got:\n{after}"
    );
    assert!(
...
async fn apps_refresh_failure_without_full_snapshot_falls_back_to_installed_apps() {
    ...
    let normalized_loading_popup = normalize_ui_text(&loading_popup);
    assert!(
        normalized_loading_popup.contains("应用正在加载已安装和可用的应用"),
    ...
    assert!(
        normalized_popup.contains("已安装。按Enter打开应用页面，可安装、管理或启用/禁用此应用。"),
...
async fn apps_popup_shows_disabled_status_for_installed_but_disabled_apps() {
    ...
    assert!(
        ...
        "expected selected app description to mention the enable/disable action after reloading, got:\n{popup}"
    );
}
...
async fn apps_initial_load_applies_enabled_state_from_requirements_with_user_override() {
    ...
    let normalized_popup = normalize_ui_text(&popup);
    assert!(
        normalized_popup.contains("已安装·已禁用。按Enter打开应用页面"),
...
async fn apps_popup_for_not_installed_app_uses_install_only_selected_description() {
    ...
    let normalized_popup = normalize_ui_text(&popup);
    assert!(
        normalized_popup.contains("可安装。按Enter打开应用页面安装此应用"),
