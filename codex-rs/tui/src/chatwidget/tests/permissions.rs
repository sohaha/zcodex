...
async fn preset_matching_accepts_workspace_write_with_extra_roots() {
    ...
    assert!(
        ...
        "WorkspaceWrite with extra roots should still match the 默认 preset"
    );
    assert!(
...
async fn approvals_popup_navigation_skips_disabled() {
    ...
    let screen = terminal.backend().vt100().screen().contents();
    assert!(
        compact_rendered_text(&screen).contains("更新权限设置"),
...
async fn permissions_selection_marks_guardian_approvals_current_after_session_configured() {
    ...
    assert!(
        compact_popup.contains("Guardian审批(当前)"),
...
async fn permissions_selection_can_disable_guardian_approvals() {
    ...
    assert!(
        ...
        "expected selecting 默认 from Guardian 审批 to switch back to manual approval review: {events:?}"
    );
    assert!(
        !events
            .iter()
...
async fn permissions_selection_sends_approvals_reviewer_in_override_turn_context() {
    ...
    let compact_popup = compact_rendered_text(&popup);
    assert!(
        compact_popup.contains("›1.默认(当前)"),
    ...
    let compact_popup = compact_rendered_text(&popup);
    assert!(
        compact_popup.contains("›2.Guardian审批"),
        "expected one Down from 默认 to select Guardian 审批: {popup}"
    );
    chat.handle_key_event(KeyEvent::from(KeyCode::Enter));
