...
#[tokio::test]
async fn experimental_popup_excludes_stable_guardian_approval() {
    let (mut chat, _rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    ...
    let normalized_popup = normalize_rendered_text(&popup);
    assert!(
        !normalized_popup.contains("Auto Review"),
        "stable auto-review feature should not be listed in experimental popup, got:\n{popup}"
    );
}
