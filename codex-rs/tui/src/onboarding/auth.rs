...
impl AuthModeWidget {
    ...
    fn render_pick_mode(&self, area: Rect, buf: &mut Buffer) {
        ...
        let chatgpt_description = if !self.is_chatgpt_login_allowed() {
            ...
            "Plus、Pro、Business 和 Enterprise 方案均包含使用额度"
        };
        let device_code_description = "在其他设备上使用一次性验证码登录";
        ...
        if !self.is_api_login_allowed() {
            lines.push(
                "  当前工作区已禁用 API Key 登录，请使用 ChatGPT 登录。"
                    .dim()
                    .into(),
            );
            lines.push("".into());
    ...
    fn render_continue_in_browser(&self, area: Rect, buf: &mut Buffer) {
        ...
        {
            lines.push("  如果链接未自动打开，请访问以下链接完成认证：".into());
            lines.push("".into());
    ...
    fn render_chatgpt_success_message(&self, area: Rect, buf: &mut Buffer) {
        let lines = vec![
            "✓ 已通过 ChatGPT 账号登录".fg(Color::Green).into(),
            "".into(),
            "  开始之前：".into(),
            "".into(),
            "  确定你希望授予 Codex 多大的自主权限".into(),
            Line::from(vec![
                "  了解更多详情请参阅 ".into(),
    ...
    fn render_api_key_entry(&self, area: Rect, buf: &mut Buffer, state: &ApiKeyInputState) {
        ...
        let mut intro_lines: Vec<Line> = vec![
            ...
            "".into(),
            "  在下方粘贴或输入你的 API Key。它将保存在本地的 auth.json 文件中。".into(),
            "".into(),
        ];
