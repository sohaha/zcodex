...
- `thread/start` — create a new thread; emits `thread/started` (including the current `thread.status`) and auto-subscribes you to turn/item events for that thread. When the request includes a `cwd` and the resolved sandbox is `workspace-write` or full access, app-server also marks that project as trusted in the user `config.toml`. Pass `sessionStartSource: "clear"` when starting a replacement thread after clearing the current session so `SessionStart` hooks receive `source: "clear"` instead of the default `"startup"`. `thread/start` also accepts an optional `federation` object to register the new thread as a local federation peer against a pre-running federation daemon; inbound federation `TextTask` envelopes are bridged into ordinary local user turns and successful completions are returned as `TextResult` envelopes. This bridge is only available on fresh `thread/start` threads and does not introduce a federation-specific `SessionSource` or `InterAgentCommunication` payload shape.
