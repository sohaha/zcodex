...
#[tokio::test]
async fn thread_read_include_turns_skips_subagent_notification_agent_messages() -> Result<()> {
    let server = create_mock_responses_server_repeating_assistant("Done").await;
    let codex_home = TempDir::new()?;
    create_config_toml(codex_home.path(), &server.uri())?;
    ...
    assert_eq!(
        turn.items.len(),
        1,
        "subagent notification agent message should stay hidden"
    );
    match &turn.items[0] {
        ThreadItem::UserMessage { content, .. } => {
            assert_eq!(
    ...
    assert!(
        !serde_json::to_string(&thread)?.contains(notification),
        "thread/read payload should not leak hidden subagent notification"
    );
