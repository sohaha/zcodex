...
- `native_memories`：原生只读 memory pipeline
- `zmemory`：可写的嵌入式长期记忆工具
...
[tui.buddy.reaction_strategy]
mode = "hybrid"                    # hybrid, ai_only, local_only
ai_probability = 0.1               # AI 生成概率（默认 0.1）
local_preference = "contextual"     # contextual, encouraging, diverse
critical_scenarios_use_ai = true   # 关键场景强制 AI
min_ai_interval_secs = 20           # AI 最小间隔秒数
min_reply_length = 100             # 短回复阈值
```
...
- [`tui/`](./tui) 当前主实现的全屏 TUI（使用 [Ratatui](https://ratatui.rs/)）。
