...
If `semantic` fails with `structuredFailure` because `language` was omitted, rerun it with an explicit `language`; that failure is not a valid structural analysis result.
