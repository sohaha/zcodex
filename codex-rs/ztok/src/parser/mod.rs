...
//! - Tier 1（Full）：完整 JSON 解析，保留全部字段
//! - Tier 2（Degraded）：部分解析并附带警告
//! - Tier 3（Passthrough）：截断原始输出并附带错误标记
//!
//! 三层体系可确保 ZTOK 不会在无提示的情况下返回错误数据。
...
pub enum ParseResult<T> {
    /// Tier 1：完整解析，包含全部结构化数据
    Full(T),

    /// Tier 2：降级解析，包含部分数据和警告
    Degraded(T, Vec<String>),

    /// Tier 3：直通原始输出，解析失败时返回截断后的原文
    Passthrough(String),
}
...
pub trait OutputParser: Sized {
    ...
    /// 2. 再尝试通过正则或文本提取部分数据
    /// 3. 最后返回带 `[ZTOK:PASSTHROUGH]` 标记的截断原始输出
...
pub fn emit_degradation_warning(tool: &str, reason: &str) {
    eprintln!("[ZTOK:DEGRADED] {tool} 解析器：{reason}");
}

/// 输出 passthrough 警告的辅助函数
pub fn emit_passthrough_warning(tool: &str, reason: &str) {
    eprintln!("[ZTOK:PASSTHROUGH] {tool} 解析器：{reason}");
}
...
mod tests {
    ...
    fn test_truncate_output() {
        ...
        assert!(truncated.contains("[ZTOK:PASSTHROUGH]"));
    ...
    fn test_truncate_output_multibyte() {
        ...
        assert!(result.contains("[ZTOK:PASSTHROUGH]"));
