...
fn run_generate(args: &[String], verbose: u8) -> Result<()> {
    ...
    if !output.status.success() {
        ...
        timer.track("prisma generate", "ztok prisma generate", &raw, &raw);
        std::process::exit(exit_code);
    }

    let filtered = filter_prisma_generate(&raw);
    println!("{filtered}");
    timer.track("prisma generate", "ztok prisma generate", &raw, &filtered);
...
fn run_migrate(subcommand: MigrateSubcommand, args: &[String], verbose: u8) -> Result<()> {
    ...
    if !output.status.success() {
        ...
        timer.track(cmd_name, &format!("ztok {cmd_name}"), &raw, &raw);
        std::process::exit(exit_code);
    }
    ...
    println!("{filtered}");
    timer.track(cmd_name, &format!("ztok {cmd_name}"), &raw, &filtered);
...
fn run_db_push(args: &[String], verbose: u8) -> Result<()> {
    ...
    if !output.status.success() {
        ...
        timer.track("prisma db push", "ztok prisma db push", &raw, &raw);
        std::process::exit(exit_code);
    }

    let filtered = filter_db_push(&raw);
    println!("{filtered}");
    timer.track("prisma db push", "ztok prisma db push", &raw, &filtered);
