...
/// # 示例
/// ```no_run
/// use ztok::utils::execute_command;
...
/// # 示例
/// ```
/// use ztok::utils::join_with_overflow;
...
/// # 示例
/// ```
/// use ztok::utils::truncate_iso_date;
...
/// # 示例
/// ```
/// use ztok::utils::ok_confirmation;
...
/// # 示例
/// ```no_run
/// use ztok::utils::detect_package_manager;
...
pub fn resolved_command(name: &str) -> Command {
    ...
    if let Err(error) = &resolved {
        eprintln!("ztok: 通过 PATH 解析 '{name}' 失败，回退到直接执行：{error}");
    }
...
mod tests {
    ...
    fn test_detect_package_manager_default() {
        // 在测试环境（ztok 仓库）中通常没有 JS lockfile，
        // 因此默认应落到 `npm`
        let pm = detect_package_manager();
