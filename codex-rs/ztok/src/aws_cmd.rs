...
fn run_sts_identity(extra_args: &[String], verbose: u8) -> Result<()> {
    ...
    if !status.success() {
        timer.track(
            ...
            "ztok aws sts get-caller-identity",
            &stderr,
            &stderr,
        );
...
fn run_s3_ls(extra_args: &[String], verbose: u8) -> Result<()> {
    ...
    if !output.status.success() {
        let stderr = crate::utils::decode_output(&output.stderr).to_string();
        timer.track("aws s3 ls", "ztok aws s3 ls", &stderr, &stderr);
        eprintln!("{}", stderr.trim());
        std::process::exit(output.status.code().unwrap_or(1));
    }
    ...
    timer.track("aws s3 ls", "ztok aws s3 ls", &raw, &filtered);
    Ok(())
}
...
fn run_ec2_describe(extra_args: &[String], verbose: u8) -> Result<()> {
    ...
    if !status.success() {
        timer.track(
            ...
            "ztok aws ec2 describe-instances",
            &stderr,
            &stderr,
        );
        std::process::exit(status.code().unwrap_or(1));
...
fn run_ecs_describe_services(extra_args: &[String], verbose: u8) -> Result<()> {
    ...
    if !status.success() {
        timer.track(
            ...
            "ztok aws ecs describe-services",
            &stderr,
            &stderr,
        );
        std::process::exit(status.code().unwrap_or(1));
...
fn run_rds_describe(extra_args: &[String], verbose: u8) -> Result<()> {
    ...
    if !status.success() {
        timer.track(
            ...
            "ztok aws rds describe-db-instances",
            &stderr,
            &stderr,
        );
        std::process::exit(status.code().unwrap_or(1));
...
fn run_cfn_list_stacks(extra_args: &[String], verbose: u8) -> Result<()> {
    ...
    if !status.success() {
        timer.track(
            ...
            "ztok aws cloudformation list-stacks",
            &stderr,
            &stderr,
        );
        std::process::exit(status.code().unwrap_or(1));
...
fn run_cfn_describe_stacks(extra_args: &[String], verbose: u8) -> Result<()> {
    ...
    if !status.success() {
        timer.track(
            ...
            "ztok aws cloudformation describe-stacks",
            &stderr,
            &stderr,
        );
        std::process::exit(status.code().unwrap_or(1));
