...
/// 检查参数中是否包含 `--json`（表示用户想要指定 JSON 字段，而不是 ZTOK 过滤）
...
pub fn run(subcommand: &str, args: &[String], verbose: u8, ultra_compact: bool) -> Result<()> {
    // 用户显式传入 --json 时，期望的是原始 gh JSON，而不是 ZTOK 过滤结果
...
fn list_prs(args: &[String], _verbose: u8, ultra_compact: bool) -> Result<()> {
    ...
    if !output.status.success() {
        ...
        timer.track("gh pr list", "ztok gh pr list", &stderr, &stderr);
    ...
    timer.track("gh pr list", "ztok gh pr list", &raw, &filtered);
    Ok(())
}
...
fn view_pr(args: &[String], _verbose: u8, ultra_compact: bool) -> Result<()> {
    ...
    if !output.status.success() {
        ...
        timer.track(
            ...
            &format!("ztok gh pr view {pr_number}"),
            &stderr,
            &stderr,
        );
        eprintln!("{}", stderr.trim());
    ...
    let line = format!("  {state} | {mergeable} {mergeable_str}\n");
    filtered.push_str(&line);
    print!("{line}");
...
fn pr_checks(args: &[String], _verbose: u8, _ultra_compact: bool) -> Result<()> {
    ...
    if !output.status.success() {
        ...
        timer.track(
            ...
            &format!("ztok gh pr checks {pr_number}"),
            &stderr,
            &stderr,
        );
        eprintln!("{}", stderr.trim());
...
fn pr_status(_verbose: u8, _ultra_compact: bool) -> Result<()> {
    ...
    if !output.status.success() {
        let stderr = crate::utils::decode_output(&output.stderr).to_string();
        timer.track("gh pr status", "ztok gh pr status", &stderr, &stderr);
        eprintln!("{}", stderr.trim());
        std::process::exit(output.status.code().unwrap_or(1));
    }
    ...
    timer.track("gh pr status", "ztok gh pr status", &raw, &filtered);
    Ok(())
}
...
fn list_issues(args: &[String], _verbose: u8, ultra_compact: bool) -> Result<()> {
    ...
    if !output.status.success() {
        let stderr = crate::utils::decode_output(&output.stderr).to_string();
        timer.track("gh issue list", "ztok gh issue list", &stderr, &stderr);
        eprintln!("{}", stderr.trim());
        std::process::exit(output.status.code().unwrap_or(1));
    }
    ...
    timer.track("gh issue list", "ztok gh issue list", &raw, &filtered);
    Ok(())
}
...
fn view_issue(args: &[String], _verbose: u8) -> Result<()> {
    ...
    if !output.status.success() {
        ...
        timer.track(
            ...
            &format!("ztok gh issue view {issue_number}"),
            &stderr,
            &stderr,
        );
        eprintln!("{}", stderr.trim());
...
fn list_runs(args: &[String], _verbose: u8, ultra_compact: bool) -> Result<()> {
    ...
    if !output.status.success() {
        let stderr = crate::utils::decode_output(&output.stderr).to_string();
        timer.track("gh run list", "ztok gh run list", &stderr, &stderr);
        eprintln!("{}", stderr.trim());
        std::process::exit(output.status.code().unwrap_or(1));
    }
    ...
    timer.track("gh run list", "ztok gh run list", &raw, &filtered);
    Ok(())
}
...
fn view_run(args: &[String], _verbose: u8) -> Result<()> {
    ...
    if !output.status.success() {
        ...
        timer.track(
            ...
            &format!("ztok gh run view {run_id}"),
            &stderr,
            &stderr,
        );
        eprintln!("{}", stderr.trim());
...
fn run_repo(args: &[String], _verbose: u8, _ultra_compact: bool) -> Result<()> {
    ...
    if !output.status.success() {
        let stderr = crate::utils::decode_output(&output.stderr).to_string();
        timer.track("gh repo view", "ztok gh repo view", &stderr, &stderr);
        eprintln!("{}", stderr.trim());
        std::process::exit(output.status.code().unwrap_or(1));
    }
    ...
    timer.track("gh repo view", "ztok gh repo view", &raw, &filtered);
    Ok(())
}
...
fn pr_create(args: &[String], _verbose: u8) -> Result<()> {
    ...
    if !output.status.success() {
        timer.track("gh pr create", "ztok gh pr create", &stderr, &stderr);
        eprintln!("{}", stderr.trim());
        std::process::exit(output.status.code().unwrap_or(1));
    }
    ...
    timer.track("gh pr create", "ztok gh pr create", &stdout, &filtered);
    Ok(())
}
...
fn pr_diff(args: &[String], _verbose: u8) -> Result<()> {
    ...
    if !output.status.success() {
        let stderr = crate::utils::decode_output(&output.stderr).to_string();
        timer.track("gh pr diff", "ztok gh pr diff", &stderr, &stderr);
        eprintln!("{}", stderr.trim());
        std::process::exit(output.status.code().unwrap_or(1));
    }
    ...
    timer.track("gh pr diff", "ztok gh pr diff", &raw, &filtered);
    Ok(())
}
...
fn run_passthrough_with_extra(cmd: &str, base_args: &[&str], extra_args: &[String]) -> Result<()> {
    ...
    timer.track_passthrough(&full_cmd, &format!("ztok {full_cmd} (passthrough)"));
...
fn run_passthrough(cmd: &str, subcommand: &str, args: &[String]) -> Result<()> {
    ...
    timer.track_passthrough(
        ...
        &format!("ztok {cmd} {subcommand} {args_str} (passthrough)"),
    );
