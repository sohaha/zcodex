...
fn run_list(depth: usize, args: &[String], verbose: u8) -> Result<()> {
    ...
    timer.track(
        ...
        &format!("ztok pnpm list --depth={depth}"),
        &stdout,
        &filtered,
    );
...
fn run_outdated(args: &[String], verbose: u8) -> Result<()> {
    ...
    timer.track("pnpm outdated", "ztok pnpm outdated", &combined, &filtered);
...
fn run_install(packages: &[String], args: &[String], verbose: u8) -> Result<()> {
    ...
    timer.track(
        ...
        &format!("ztok pnpm install {}", packages.join(" ")),
        &combined,
        &filtered,
    );
...
pub fn run_passthrough(args: &[OsString], verbose: u8) -> Result<()> {
    ...
    timer.track_passthrough(
        ...
        &format!("ztok pnpm {args_str} (passthrough)"),
    );
