...
fn docker_ps(_verbose: u8) -> Result<()> {
    ...
    if !output.status.success() {
        ...
        eprint!("{stderr}");
        timer.track("docker ps", "ztok docker ps", &raw, &raw);
    ...
    if stdout.trim().is_empty() {
        ztok.push_str("🐳 0 个容器");
        println!("{ztok}");
        timer.track("docker ps", "ztok docker ps", &raw, &ztok);
        return Ok(());
    }

    let count = stdout.lines().count();
    ztok.push_str(&format!("🐳 {count} 个容器：\n"));
    ...
    for line in stdout.lines().take(15) {
        ...
        if parts.len() >= 4 {
            ...
            if ports == "-" {
                ztok.push_str(&format!("  {id} {name} ({short_image})\n"));
            } else {
                ztok.push_str(&format!("  {id} {name} ({short_image}) [{ports}]\n"));
            }
        }
    }
    if count > 15 {
        ztok.push_str(&format!("  ... +{} 个", count - 15));
    }
...
fn docker_images(_verbose: u8) -> Result<()> {
    ...
    if !output.status.success() {
        let stderr = crate::utils::decode_output(&output.stderr);
        eprint!("{stderr}");
        timer.track("docker images", "ztok docker images", &raw, &raw);
        std::process::exit(output.status.code().unwrap_or(1));
    }
    ...
    if lines.is_empty() {
        ztok.push_str("🐳 0 个镜像");
        println!("{ztok}");
        timer.track("docker images", "ztok docker images", &raw, &ztok);
        return Ok(());
    }
    ...
    for line in lines.iter().take(15) {
        ...
        if !parts.is_empty() {
            ...
            ztok.push_str(&format!("  {short} [{size}]\n"));
        }
    }
    if lines.len() > 15 {
        ztok.push_str(&format!("  ... +{} 个", lines.len() - 15));
    }
...
fn docker_logs(args: &[String], _verbose: u8) -> Result<()> {
    ...
    if container.is_empty() {
        println!("用法：ztok docker logs <container>");
        return Ok(());
    }
...
fn kubectl_pods(args: &[String], _verbose: u8) -> Result<()> {
    ...
    if !issues.is_empty() {
        ...
        if issues.len() > 10 {
            ztok.push_str(&format!("  ... +{} 个", issues.len() - 10));
        }
    }
...
fn kubectl_services(args: &[String], _verbose: u8) -> Result<()> {
    ...
    if services.len() > 15 {
        ztok.push_str(&format!("  ... +{} 个", services.len() - 15));
    }
...
fn kubectl_logs(args: &[String], _verbose: u8) -> Result<()> {
    ...
    if pod.is_empty() {
        println!("用法：ztok kubectl logs <pod>");
        return Ok(());
    }
...
pub fn run_docker_passthrough(args: &[OsString], verbose: u8) -> Result<()> {
    ...
    timer.track_passthrough(
        ...
        &format!("ztok docker {args_str} (passthrough)"),
    );
...
pub fn run_compose_ps(verbose: u8) -> Result<()> {
    ...
    let ztok = format_compose_ps(&structured);
    println!("{ztok}");
    timer.track("docker compose ps", "ztok docker compose ps", &raw, &ztok);
    Ok(())
}
...
pub fn run_compose_build(service: Option<&str>, verbose: u8) -> Result<()> {
    ...
    let ztok = format_compose_build(&raw);
    println!("{ztok}");
    let svc_label = service.unwrap_or("all");
    timer.track(
        &format!("docker compose build {svc_label}"),
        "ztok docker compose build",
        &raw,
        &ztok,
    );
    Ok(())
...
pub fn run_compose_passthrough(args: &[OsString], verbose: u8) -> Result<()> {
    ...
    timer.track_passthrough(
        ...
        &format!("ztok docker compose {args_str} (passthrough)"),
    );
...
pub fn run_kubectl_passthrough(args: &[OsString], verbose: u8) -> Result<()> {
    ...
    timer.track_passthrough(
        ...
        &format!("ztok kubectl {args_str} (passthrough)"),
    );
