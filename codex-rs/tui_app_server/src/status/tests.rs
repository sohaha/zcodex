...
fn sanitize_directory(lines: Vec<String>) -> Vec<String> {
    lines
        .into_iter()
        .map(|line| {
            let line = line.replace(&format!("(v{CODEX_CLI_VERSION})"), "(v1.0.0)");
...
async fn status_permissions_non_default_workspace_write_is_custom() {
    ...
    let permissions_text = permissions_line
        .split(if permissions_line.contains("Permissions:") {
            "Permissions:"
        } else {
            "权限:"
        })
...
async fn status_snapshot_shows_unlimited_credits() {
    ...
    assert!(
        rendered
            .iter()
            .any(|line| (line.contains("Credits:") || line.contains("积分:"))
                && (line.contains("Unlimited") || line.contains("无限"))),
...
async fn status_snapshot_shows_positive_credits() {
    ...
    assert!(
        rendered
            .iter()
            .any(|line| (line.contains("Credits:") || line.contains("积分:"))
                && (line.contains("13 credits") || line.contains("13 积分"))),
...
async fn status_context_window_uses_last_usage() {
    ...
    assert!(
        context_line.contains("13.7K used / 272K") || context_line.contains("13.7K 已用 / 272K"),
