...
/// Enabling the Guardian 审批 experiment in the TUI should also switch the
/// current `/approvals` settings to the matching Guardian 审批 mode. Users
...
impl App {
    ...
    async fn update_feature_flags(&mut self, updates: Vec<(Feature, bool)>) {
        ...
        // Guardian 审批 owns `approvals_reviewer`, but disabling the feature
    ...
    async fn submit_thread_op(
        ...
        self.chat_widget.add_error_message(format!(
            "app-server TUI 暂不支持线程 {thread_id} 的此操作。"
        ));
        Ok(())
    }
    ...
    async fn select_agent_thread(
        ...
        if !self.thread_event_channels.contains_key(&thread_id) {
            self.chat_widget
                .add_error_message(format!("连接到智能体线程 {thread_id} 失败。"));
            return Ok(());
        }
    ...
    async fn start_fresh_session_with_summary_hint(
        ...
        match app_server.start_thread(&config).await {
            Ok(started) => {
                ...
                {
                    self.chat_widget
                        .add_error_message(format!("连接到新的 app-server 线程失败：{err}"));
