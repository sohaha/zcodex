...
async fn unknown_model_warning_is_localized_for_fallback_metadata() {
    ...
    let warning_event = rx_event.recv().await.expect("warning event should arrive");
    match warning_event.msg {
        EventMsg::Warning(WarningEvent { message }) => {
            assert_eq!(
                message,
                "未找到模型 `mystery-model` 的元数据，已改用兜底元数据；这可能降低性能并导致异常。"
            );
        }
...
async fn turn_start_zmemory_recall_note_is_produced_for_regular_user_turns() {
    // Regression anchor for pending input stable-preference recall side effects.
}

#[tokio::test]
async fn pending_user_input_neutral_steer_preserves_existing_tldr_directives() {
    // Regression anchor for pending input tool routing directive merging.
}
