...
async fn run_guardian_review(
    ...
    let assessment = match outcome {
        ...
        GuardianReviewOutcome::TimedOut => {
            let rationale = "自动批准审查在评估本次批准请求时超时。".to_string();
            session
                .send_event(
                    turn.as_ref(),
    ...
    let user_authorization = match assessment.user_authorization {
        GuardianUserAuthorization::Unknown => "未知",
        GuardianUserAuthorization::Low => "低",
        GuardianUserAuthorization::Medium => "中",
        GuardianUserAuthorization::High => "高",
    };
    let warning = format!(
        "自动批准审查{verdict}（风险：{}，授权：{user_authorization}）：{}",
