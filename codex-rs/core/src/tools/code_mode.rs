...
pub(crate) fn instructions(config: &Config) -> Option<String> {
    ...
    section.push_str("- Import nested tools from `tools.js`, for example `import { exec_command } from \"tools.js\"` or `import { tools, ALL_TOOLS } from \"tools.js\"` to inspect the available `{ module, name, description }` entries. Valid identifier tool names are exposed as named exports; every tool also remains available via `tools[name]` for compatibility. Namespaced tools are also available from `tools/<namespace...>.js`; MCP tools use `tools/mcp/<server>.js`, for example `import { append_notebook_logs_chart } from \"tools/mcp/ologs.js\"`. Nested tool calls resolve to their code-mode result values.\n");
    section.push_str(&format!(
