...
mod handlers {
            ...
            let cwd_abs = match AbsolutePathBuf::try_from(cwd.as_path()) {
                ...
                Err(err) => {
                    ...
                    skills.push(SkillsListEntry {
                        ...
                        skills: Vec::new(),
                        errors: vec![codex_protocol::protocol::SkillErrorInfo {
                            path: cwd_for_entry,
                            message,
                        }],
                    });
    ...
    pub(super) async fn persist_thread_memory_mode_update(
        ...
        .ok_or_else(|| anyhow::anyhow!("会话持久化已禁用，无法更新线程记忆模式。"))?;
        recorder.persist().await?;
        recorder.flush().await?;
...
async fn run_sampling_request(
    ...
    loop {
        ...
        if should_short_circuit_to_parent_model(&sampling_turn_context, &err)
            && let Some(parent_model) = subagent_parent_model(&sampling_turn_context)
        {
            let parent_model_turn_context = Arc::new(
                sampling_turn_context
                    .with_model(parent_model.to_string(), &sess.services.models_manager)
                    .await,
            );
            let fallback_model = parent_model_turn_context.model_info.slug.clone();
            sampling_turn_context = parent_model_turn_context;
            ...
            sess.send_event(
                ...
                EventMsg::Warning(WarningEvent {
                    message: format!(
                        "Subagent requested model failed; retrying with parent model `{fallback_model}`. {err:#}"
                    ),
                }),
            )
            .await;
