...
- Use built-in `rtk_*` tools (`rtk_read`, `rtk_grep`, `rtk_find`, `rtk_diff`, `rtk_json`, `rtk_deps`, `rtk_log`, `rtk_ls`, `rtk_tree`, `rtk_wc`, `rtk_summary`, `rtk_err`) for token-efficient inspection, search, listing, counting, and concise diff/error summaries.
