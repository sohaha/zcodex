...
impl PathRow {
    pub fn root(domain: String) -> Self {
        let _ = domain;
        Self {
            edge_id: 0,
...
/// Shared statistical query functions used by stats_action, doctor, and system_views.
pub(crate) mod stats_queries {
    use anyhow::Result;
    use rusqlite::Connection;

    pub fn alias_node_count(conn: &Connection) -> Result<i64> {
        Ok(conn.query_row(
            "SELECT COUNT(*) FROM (
                 SELECT child_uuid
                 FROM edges e
    ...
    pub fn trigger_node_count(conn: &Connection) -> Result<i64> {
        Ok(conn.query_row(
    ...
    pub fn alias_nodes_missing_triggers(conn: &Connection) -> Result<i64> {
        Ok(conn.query_row(
            "SELECT COUNT(*) FROM (
                 SELECT e.child_uuid
                 FROM edges e
    ...
    pub fn paths_missing_disclosure(conn: &Connection) -> Result<i64> {
        Ok(conn.query_row(
    ...
    pub fn disclosures_needing_review(conn: &Connection) -> Result<i64> {
        Ok(conn.query_row(
            "SELECT COUNT(*) FROM edges e
             JOIN paths p ON p.edge_id = e.id
    ...
    pub fn orphaned_memory_count(conn: &Connection) -> Result<i64> {
        Ok(conn.query_row(
    ...
    pub fn deprecated_memory_count(conn: &Connection) -> Result<i64> {
        Ok(conn.query_row(
    ...
    pub fn search_document_count(conn: &Connection) -> Result<i64> {
        Ok(
    ...
    pub fn fts_document_count(conn: &Connection) -> Result<i64> {
        Ok(
...
pub(crate) mod domain_checks {
    use crate::config::ZmemoryConfig;
    use crate::schema::ensure_domain_root;
    use anyhow::Result;
    use rusqlite::Connection;

    pub fn ensure_readable_domain(
...
pub(crate) mod db_helpers {
    ...
    pub fn find_path_row(conn: &Connection, uri: &ZmemoryUri) -> Result<Option<PathRow>> {
        if uri.is_root() {
    ...
    pub fn read_active_memory(conn: &Connection, node_uuid: &str) -> Result<Option<MemoryRow>> {
    ...
    pub fn load_keywords(conn: &Connection, node_uuid: &str) -> Result<Vec<String>> {
        let mut stmt = conn.prepare(
    ...
    pub fn count_aliases(conn: &Connection, node_uuid: &str) -> Result<i64> {
        conn.query_row(
