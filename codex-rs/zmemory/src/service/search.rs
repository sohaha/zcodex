//! Search action, FTS query normalization, and text processing utilities.
...
pub(crate) fn search_action(
    ...
    let raw_matches = if let Some(scope) = scope {
        ...
        let mut stmt = conn.prepare(&sql)?;
        stmt.query_map(params![normalized_query], row_to_search_match)?
            .collect::<rusqlite::Result<Vec<_>>>()?
    };
...
fn row_to_search_match(row: &rusqlite::Row<'_>) -> rusqlite::Result<SearchMatch> {
    Ok(SearchMatch {
        domain: row.get(0)?,
        path: row.get(1)?,
        uri: row.get(2)?,
...
mod tests {
    ...
    #[test]
    fn fts5_special_tokens_are_escaped() {
        // FTS5 operators should not pass through unchanged
        assert_eq!(normalize_search_query("AND"), "and");
        assert_eq!(normalize_search_query("foo AND bar"), "foo and bar");
        assert_eq!(normalize_search_query("NOT test"), "not test");
        assert_eq!(normalize_search_query("a OR b"), "a or b");
        assert_eq!(normalize_search_query("NEAR(x)"), "near x");
        assert_eq!(normalize_search_query("test*"), "test");
    }

    #[test]
    fn cjk_query_passes_through_normalize_field() {
        assert_eq!(
            normalize_search_query("\u{91cf}\u{5b50}\u{6bd4}\u{7279}"),
            "\u{91cf}\u{5b50}\u{6bd4}\u{7279}"
        );
    }

    #[test]
    fn ascii_query_splits_on_separators() {
        let result = normalize_search_query("foo-bar:baz");
