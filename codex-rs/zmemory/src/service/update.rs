//! Update action for the zmemory service layer.
...
use super::common::normalize_optional_text;
...
pub(crate) fn update_action(
    ...
    // Early bail: check before opening a transaction
    if updated_content.is_none() && args.priority.is_none() && args.disclosure.is_none() {
    ...
    if let Some(content) = updated_content {
        if content != current_memory.content {
            tx.execute(
...
fn resolve_updated_content(
    ...
    match (
        ...
        (Some(content), None, None, None) => Ok(Some(
